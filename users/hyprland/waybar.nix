  ...
  programs.waybar = {
    ...
    settings = {
      mainBar = {
        ...
        modules-right = [ "cpu" "memory" "backlight" "pulseaudio" "tray" "battery" ];

        "custom/nix" = {
          "format" = "   ";
          "tooltip" = false;
          "on-click" = "rofi  -show drun  -show-icons -location 1";
        };
        "hyprland/workspaces" = {
          "format" = "{icon}";
          "all-outputs" = true;
    ...
    style = ''
      * {
      ...
      #clock, #backlight, #pulseaudio, #bluetooth, #network, #battery, #cpu, #memory, #tray{
        border-radius: 10px;
        background-color: #11111b;
        color: #cdd6f4;
        margin-top: 15px;
